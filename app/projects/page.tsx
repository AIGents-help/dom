  ...
  return (
    <div className="bg-background text-ink">
      <section className="relative min-h-[520px] overflow-hidden border-b border-white/10">
        <Image src="/images/construction-aerial.jpg" alt="Aerial mapping project evidence" fill priority sizes="100vw" className="object-cover" />
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {domKnowledge.evidence.map((item) => (
            <article key={item.slug} className="group overflow-hidden rounded-3xl border border-white/10 bg-[#111923] shadow-xl transition hover:-translate-y-1 hover:border-accent/50 hover:shadow-2xl">
              <div className="relative h-64 overflow-hidden"><Image src="/images/construction-aerial.jpg" alt="" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#111923] via-transparent to-transparent" /><div className="absolute left-6 top-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-2 text-xs font-black uppercase tracking-[.16em] text-white backdrop-blur"><FlaskConical className="h-4 w-4 text-accent" />{item.type}</div></div>
