...
type Order = { id: string; order_number: string; status: string; payment_status: string; customer_name: string | null; customer_email: string | null; customer_phone: string | null; total_cents: number; shipping_address: Record<string, string> | null; tracking_carrier: string | null; tracking_number: string | null; created_at: string; shop_order_items: Item[] };
...
export default function AdminOrdersPage() {
  ...
  const request = useCallback(async (init?: RequestInit) => {
    ...
    const response = await fetch("/api/admin/orders", { ...init, headers: { "Content-Type": "application/json", Authorization: `Bearer ${data.session.access_token}`, ...(init?.headers ?? {}) } });
...
function InventoryRow({ item, busy, save }: { item: Inventory; busy: boolean; save: (payload: Record<string, unknown>) => void }) {
  ...
  return <div className="card grid gap-3 p-4 lg:grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] lg:items-end"><strong>{item.product_name}</strong><label className="text-xs">Mode<select value={mode} onChange={(event) => setMode(event.target.value)} className="mt-1 w-full rounded border p-2 text-sm"><option value="made_to_order">Made to order</option><option value="stocked">Stocked</option></select></label><label className="text-xs">Available<input type="number" min="0" disabled={mode !== "stocked"} value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} className="mt-1 w-full rounded border p-2 text-sm" /></label><label className="text-xs">Base shipping ($)<input type="number" min="0" step="0.01" value={(base / 100).toFixed(2)} onChange={(event) => setBase(Math.round(Number(event.target.value) * 100))} className="mt-1 w-full rounded border p-2 text-sm" /></label><label className="text-xs">Each extra ($)<input type="number" min="0" step="0.01" value={(additional / 100).toFixed(2)} onChange={(event) => setAdditional(Math.round(Number(event.target.value) * 100))} className="mt-1 w-full rounded border p-2 text-sm" /></label><div className="flex items-center gap-3"><label className="text-xs"><input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)} /> Active</label><button disabled={busy} onClick={() => save({ fulfillmentMode: mode, availableQuantity: quantity, shippingBaseCents: base, shippingAdditionalCents: additional, active })} className="rounded bg-slate-900 px-3 py-2 text-sm font-bold text-white disabled:opacity-50">Save</button></div></div>;
}
