-- Make pilot-owner delivery revision-aware.
-- Only current deliverables are released. Historical/superseded versions remain
-- immutable, and corrected outputs supersede their predecessor only when the
-- mission owner certifies the current package for delivery.
...
begin
  ...
  if not exists (
    ...
    where job_id = v_job.id
      and coalesce(client_status, '') <> 'superseded'
  ) then
  ...
  -- Release only active/current deliverables. Superseded history stays untouched.
  ...
  -- If an active corrected version replaces a client-requested revision,
  -- hand off that lineage only now, at owner certification.
  ...
  insert into public.mission_activity_events (
    ...
    jsonb_build_object(
      'approval_authority', 'pilot_mission_owner',
      'dom_qc_required', false,
      'revision_aware_delivery', true
    )
  );
end;
$function$;
