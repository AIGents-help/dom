...
const HERO_PAYLOAD = "/brand/dom-safety-hero/ultra.txt?v=20260920-1";
...
export default function HomeSafetyHero() {
  ...
  useEffect(() => {
    ...
    fetch(HERO_PAYLOAD, { cache: "force-cache" })
      .then((response) => {
        if (!response.ok) {
          throw new Error("DOM safety hero payload failed to load");
        }
        return response.text();
      })
      .then((payload) => {
        if (!cancelled) {
          setSrc(`data:image/jpeg;base64,${payload.replace(/\s/g, "")}`);
        }
      })
      .catch(() => {
