...
async function context(req: NextRequest, assignmentId: string) {
  ...
  const { data: contractor } = await admin.from("contractors").select("id,insurance_verified,insurance_provider,insurance_policy_number,insurance_expires_on,insurance_liability_cents,dom_gig_insurance_eligible").eq("user_id", user.id).maybeSingle(); if (!contractor) return null;
  const { data: assignment } = await admin.from("mission_assignments").select("id,job_id,contractor_id,status,insurance_source,mission_insurance_verified,mission_insurance_reference,mission_insurance_expires_at,job:jobs(mission_request_id,checked_in_at,started_at,completed_at)").eq("id", assignmentId).eq("contractor_id", contractor.id).maybeSingle();
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ assignmentId: string }> }) {
  ...
  return NextResponse.json({ items: items ?? [], job, insurance });
}
