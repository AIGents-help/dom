...
declare
  ...
  v_reference_total_cents integer;
  v_billing_mode text;
  v_no_charge_reason text;
...
begin
  ...
  v_reference_total_cents := coalesce((p_quote ->> 'referenceTotalCents')::integer, v_total_cents);
  v_billing_mode := coalesce(nullif(trim(p_quote ->> 'billingMode'), ''), 'paid');
  v_no_charge_reason := nullif(trim(coalesce(p_quote ->> 'noChargeReason', '')), '');

  if v_total_cents is null or v_total_cents < 0 then
    raise exception 'invalid mission price';
  end if;

  if v_billing_mode = 'paid' then
    if v_total_cents <= 0 then
      raise exception 'paid missions require a positive price';
    end if;
  elsif v_billing_mode = 'no_charge' then
    if v_total_cents <> 0 then
      raise exception 'no-charge missions must have a zero client price';
    end if;
    if v_no_charge_reason is null then
      raise exception 'no-charge missions require a reason';
    end if;
    if v_reference_total_cents is null or v_reference_total_cents <= 0 then
      raise exception 'no-charge missions require a positive reference value';
    end if;
  else
    raise exception 'invalid billing mode';
  end if;
  ...
  if v_billing_mode = 'no_charge' then
    ...
    values(
      ...
      'pilot', 'shared', 'no_charge_mission_created',
      'Pilot intentionally created a no-charge mission',
      jsonb_build_object(
        'reason', v_no_charge_reason,
        'reference_total_cents', v_reference_total_cents,
